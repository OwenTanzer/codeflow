...
export function adaptFunctionAnalysis({ context, entrySymbol, source, flowchartIR, analyzer }) {
  ...
  const nodes = flowchartIR.nodes.map((node) => {
    ...
    if (!synthetic && !node.location) {
      warnings.push(`Node "${node.id}" (${node.label}) has no source location; its coordinate could not be attached.`);
    }
    return {
      id: node.id,
      layer: 'function',
  ...
  const edges = flowchartIR.edges.map((edge, i) => {
    ...
    if (edge.label && kind === 'flow') {
      ...
      graphEdge.metadata = { label: edge.label };
    }
    return graphEdge;
  });
  ...
  return makeGraphIR({
    ...
    metadata: { flowchartTitle: flowchartIR.title || null, functionRange: flowchartIR.functionRange || null },
  });
}
