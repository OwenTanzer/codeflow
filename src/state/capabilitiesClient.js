...
// the UI fetches this once per committed app password and uses it to hide/
...
//
// MOO-72 Commit 8 PR review: the app password is not a configuration version --
// an operator can toggle these flags and redeploy without ever changing the
// password, so caching keyed only by password meant a long-lived tab could
// never observe a rollout/rollback (most notably: after a layer is
// re-enabled, a page that had cached `false` kept refusing to call it until
// a full reload). A short TTL bounds how stale the cached value can ever be,
 ...
 */
export async function fetchCapabilities(appPassword, now = Date.now) {
  if (cached && cachedForPassword === appPassword && now() - cachedAt < CAPABILITIES_CACHE_TTL_MS) {
    return cached;
  }
  try {
